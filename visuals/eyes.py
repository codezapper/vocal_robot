# Adapted from https://github.com/sofianhw/RoboEyes
...
class RoboEyes:
    ...
    def __init__(self, draw_surface, window, width=320, height=240, frame_rate=50):
        """
        ...
        # Eye Geometry
        self.space_between_default = 50  # Reduced space for larger eyes
        self.space_between_current = self.space_between_default
        self.space_between_next = self.space_between_default

        # Left Eye
        self.eyeLwidth_default = 300   # Increased size proportionally
        self.eyeLheight_default = 300
    ...
    def setMood(self, mood):
        if mood == self.moods["TIRED"]:
            self.tired = True
            self.angry = False
            self.happy = False
        elif mood == self.moods["ANGRY"]:
            self.tired = False
            self.angry = True
            self.happy = False
        elif mood == self.moods["HAPPY"]:
            self.tired = False
            self.angry = False
            self.happy = True
        else:
    ...
    def drawEyes(self):
        ...
        MAINCOLOR = self.colors["FOREGROUND"]
        current_time = pygame.time.get_ticks()

        self.eyeLx_next = self.getScreenConstraint_X() // 2
        self.eyeLy_next = self.getScreenConstraint_Y() // 2
        self.eyeRx_next = self.getScreenConstraint_X() // 2
        self.eyeRy_next = self.getScreenConstraint_Y() // 2
    ...
    def draw_eye(self, x, y, width, height, border_radius, color):
        ...
        # pygame.draw.circle(self.surface, color, (((x+width)//2, (y+height)//2)), 30)
        if border_radius > 0:
    ...
    def keep_going(self, commands_queue, clock):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_1:
                    self.setMood(self.moods["TIRED"])
                elif event.key == pygame.K_2:
                    self.setMood(self.moods["ANGRY"])
                elif event.key == pygame.K_3:
                    self.setMood(self.moods["HAPPY"])
                elif event.key == pygame.K_0:
                    self.setMood(self.moods["DEFAULT"])
        ...
        rotated_rect = self.surface.get_rect(center=self.window.get_rect().center)

        # Clear the self.window before blitting
        self.window.fill(self.colors["BACKGROUND"])

        # Blit the rotated surface onto the main self.window
        self.window.blit(self.surface, rotated_rect)
